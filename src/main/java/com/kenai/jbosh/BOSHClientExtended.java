 ...
 *     .build();
 * BOSHClientExtended client = BOSHClientExtended.create(cfg);
 * </pre>
 * Additional client configuration options are available.  See the
 * {@code BOSHClientConfig.Builder} class for more information.
 * <p/>
 * Once a {@code BOSHClientExtended} instance has been created, communication with
 ...
 * @see BOSHClientExtendedResponseListener
 * @see BOSHClientConnListener
 * @see ComposableBody.Builder
 */
public final class BOSHClientExtended {

    /**
     * Logger.
     */
    private static final Logger LOG = Logger.getLogger(
            BOSHClientExtended.class.getName());
    ...
    private static final int EMPTY_REQUEST_DELAY = Integer.getInteger(
            BOSHClientExtended.class.getName() + ".emptyRequestDelay",
    ...
    private static final int PAUSE_MARGIN = Integer.getInteger(
    		BOSHClientExtended.class.getName() + ".pauseMargin",
     ...
     */
    private final Set<BOSHClientExtendedConnListener> connListeners =
            new CopyOnWriteArraySet<BOSHClientExtendedConnListener>();

    /**
     * Request listeners.
     */
    private final Set<BOSHClientExtendedRequestListener> requestListeners =
            new CopyOnWriteArraySet<BOSHClientExtendedRequestListener>();

    /**
     * Response listeners.
     */
    private final Set<BOSHClientExtendedResponseListener> responseListeners =
            new CopyOnWriteArraySet<BOSHClientExtendedResponseListener>();
    ...
    static {
        final String prop =
                BOSHClientExtended.class.getSimpleName() + ".assertionsEnabled";
     ...
     */
    private BOSHClientExtended(final BOSHClientConfig sessCfg) {
        cfg = sessCfg;
        init();
    }
     ...
     */
    public static BOSHClientExtended create(final BOSHClientConfig clientCfg) {
        if (clientCfg == null) {
            throw(new IllegalArgumentException(
                    "Client configuration may not be null"));
        }
        return new BOSHClientExtended(clientCfg);
    }
    ...
    public void addBOSHClientConnListener(
            final BOSHClientExtendedConnListener listener) {
        if (listener == null) {
    ...
    public void addBOSHClientRequestListener(
            final BOSHClientExtendedRequestListener listener) {
        if (listener == null) {
            throw(new IllegalArgumentException(NULL_LISTENER));
        }
    ...
    public void addBOSHClientResponseListener(
            final BOSHClientExtendedResponseListener listener) {
        if (listener == null) {
            throw(new IllegalArgumentException(NULL_LISTENER));
        }
    ...
    private void init() {
        ...
        try {
            ...
            procThread.setName(BOSHClientExtended.class.getSimpleName()
    ...
    private void fireRequestSent(final AbstractBody request) {
        ...
        BOSHMessageEventExtended event = null;
        for (BOSHClientExtendedRequestListener listener : requestListeners) {
            if (event == null) {
                event = BOSHMessageEventExtended.createRequestSentEvent(this, request);
            }
            try {
    ...
    private void fireResponseReceived(final AbstractBody response) {
        ...
        BOSHMessageEventExtended event = null;
        for (BOSHClientExtendedResponseListener listener : responseListeners) {
            if (event == null) {
                event = BOSHMessageEventExtended.createResponseReceivedEvent(
                        this, response);
            }
            try {
    ...
    private void fireConnectionEstablished() {
        ...
        try {
            BOSHClientExtendedConnEvent event = null;
            for (BOSHClientExtendedConnListener listener : connListeners) {
                if (event == null) {
                    event = BOSHClientExtendedConnEvent
    ...
    private void fireConnectionClosed() {
        ...
        for (BOSHClientExtendedConnListener listener : connListeners) {
            if (event == null) {
                event = BOSHClientExtendedConnEvent.createConnectionClosedEvent(this);
            }
            try {
                listener.connectionEvent(event);
            } catch (Exception ex) {
